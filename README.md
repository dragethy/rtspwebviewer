...
2. Deploy container:
```bash
$ docker run --name rtspwebviewer --user docker rtspwebviewer:latest '/bin/zsh -c "source $HOME/.zshrc && python -m rtspwebviewer.run -u <rtsp_address> -a <listening_ip_address> -p <port> -t <web_title>"'
```
